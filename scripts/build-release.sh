...
for architecture in amd64 arm64; do
    ...
    (
        ...
        CGO_ENABLED=0 GOOS=linux GOARCH="$architecture" \
            go build -trimpath -ldflags='-s -w' -o "$package_directory/ampcontrol" ./cmd/ampcontrol
    )
    ...
    install -m 0755 "$PROJECT_DIRECTORY/scripts/install.sh" "$PROJECT_DIRECTORY/scripts/ampcontrol-amp" "$package_directory/scripts/"
